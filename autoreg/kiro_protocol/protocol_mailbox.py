...
from kiro_protocol.core import KiroRegister, _pwd, wait_for_otp




def _human_pause(name: str, lo: float, hi: float, log) -> None:
    t = random.uniform(lo, hi)
    log(f"  [pause] {name}: {t:.1f}s")
    time.sleep(t)
...
class KiroProtocolMailboxWorker:
    ...
    def run(
        ...
        *,
        ...
        _human_pause("land-signin", 2.5, 6.5, self.client.log)
        if not self.client.step3_signin_flow(email):
            raise RuntimeError("signin flow失败")
        _human_pause("think-signup", 1.5, 4.0, self.client.log)
        ...
        _human_pause("read-otp-prompt", 3.0, 8.0, self.client.log)
        ...
        _human_pause("type-password", 8.0, 20.0, self.client.log)
        ...
        _human_pause("redirect-login", 1.5, 4.0, self.client.log)
