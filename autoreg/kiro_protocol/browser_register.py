...
def _type_input_humanlike(page, selectors, value, timeout=20):
    """Type characters one-by-one with human-realistic 70-220ms delays.
    Used for fields where AWS/Kiro do keystroke-timing analytics (email field).
    """
    deadline = time.time() + timeout
    while time.time() < deadline:
        for sel in selectors:
            try:
                ...
                if el and el.is_visible():
                    ...
                    for ch in value:
                        el.type(ch, delay=random.randint(70, 220))
                    return True
            except Exception:
                pass
        time.sleep(0.5)
...
class KiroBrowserRegister:
    ...
    def _handle_aws_profile_spa(self, page, email: str, password: str) -> None:
        ...
        while time.time() < deadline:
            ...
            # --- create-password 步 ---
            if ("create-password" in hash_part or "set-password" in hash_part) and "create-password" not in handled_steps:
    ...
    def run(self, email: str, password: str) -> dict:
        ...
        # Enable geoip auto-rotation to match proxy egress location so Camoufox
        # advertises a timezone/locale/screen consistent with the proxy IP
        # (Builder ID fraud filter cross-checks IP geo vs navigator props).
        if proxy:
        ...
        with Camoufox(**launch_opts) as browser:
            ...
            # Realistic dwell: humans hover/read for 5-12 sec before clicking
            # an SSO provider. Kiro tracks page-event timing during this window.
            try:
                page.mouse.move(420, 360)
                time.sleep(random.uniform(1.0, 2.0))
                page.mouse.move(380, 460, steps=10)
                time.sleep(random.uniform(0.8, 1.5))
                # Scroll a tiny bit then back — lots of users skim before signin
            ...
            if AWS_SIGNIN_DOMAIN in page.url:
                ...
                # Slow per-character typing on email — humans don't paste full strings
                # into the AWS Builder ID signin page; they type at 80-200ms/char.
                if not _type_input_humanlike(page, email_selectors, email, timeout=15):
                    raise RuntimeError(f"未找到邮箱输入框: {page.url}")
                # Pause before clicking Continue — reading prompt
